...
void CMFCApplication1Dlg::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	CString str;
	for (int i = 0; i < 5; i++) {
		str.Format(L"%d번 메뉴", i);
		menu.AppendMenu(MF_STRING, 20000 + i, str);	// WM_COMMAND가 발새하고 wParam에서 메시지번호를 확인해야함.
	}
...
BOOL CMFCApplication1Dlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (cmd_id > 20000 && cmd_id < 20005) {
		CString str;
		str.Format(L"%d 항목을 선택했습니다.", cmd_id - 20000);
		AfxMessageBox(str);
	}
